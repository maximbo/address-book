...
class AddressBook:
    ...
    async def create(self, phone: str, address: str) -> str:
        if not await self._redis.hsetnx(self.REDIS_KEY, phone, address):
            raise ObjectAlreadyExists

        return address

    async def get(self, phone: str) -> str:
        address = await self._redis.hget(self.REDIS_KEY, phone)
        if address is None:
    ...
    async def update(self, phone: str, address: str):
        if not await self._redis.hexists(self.REDIS_KEY, phone):
            raise ObjectNotFound

        await self._redis.hset(self.REDIS_KEY, phone, address)

        return address

    async def delete(self, phone: str):
        if not await self._redis.hdel(self.REDIS_KEY, phone):
            raise ObjectNotFound
